...
import { getArticlesByType } from '@/api/index.js'
import { showError } from '@/utils/message.js'

const route = useRoute()
const router = useRouter()

const type_id = ref(undefined)
const label_id = ref(undefined)
const data = ref([])
const tagWithNum = ref(null)
const pageSize = ref(5)
const count = ref(0)

async function fetchData(page) {
    type_id.value = route.params.type_id
    label_id.value = route.params.label_id
    ...
    if (label_id.value !== undefined) {
        params.label_id = label_id.value
    }

    try {
        const response = await getArticlesByType(params)
        data.value = response.data.articles
        tagWithNum.value = response.data.tag_with_num
...
function selectTag(selectedTypeId, selectedLabelId) {
    router.push({
...
watch(() => route.params, (to, from) => {
    if (to.type_id !== from.type_id || to.label_id !== from.label_id) {
        fetchData(1)
    }
}, { deep: true })
</script>
            ...
            <div>
                <div class="my-animation-slide-top">
                    <div class="poem-container myCenter my-animation-hideToShow">
                        <el-image style="position: absolute; margin-top: -50px;" class="my-el-image poem-image">
                        ...
                        <div class="poem-wrap">
                            ...
                            <p class="poem">水面的涟漪虽然暂时消失了，但仍会有人记得，风曾经吹过。</p>
                        </div>
                    </div>
                </div>
                <div class="my-animation-hideToShow" style="background: #fff; padding-top: 40px;">
                    <div class="sort-warp shadow-box">
                        <div v-for="item in tagWithNum">
                            <div class="myTag" style="background: black; margin: 12px;"
                                @click="selectTag(type_id, item.id)">
                                <span class="name">
                    ...
                    <div class="article-wrap">
                        <div class="recent-post-container" :key="`article-${type_id}-${label_id}`">
                            ...
                            <div v-for="(item, index) in data" @click="showArticle(item.id)"
                                ...
                                <div class="recent-post-item-post"
                                    ...
                                    <h3 class="el-tooltip" aria-describedby="el-tooltip-9173" tabindex="0">
                                        {{ item.title }}</h3>
                        ...
                        <div class="pagination">
                            <div class="myCenter proPage" style="margin: 15px 0px;">
                                <el-pagination background layout="prev, pager, next" :page-size="pageSize"
                                    :total="count" :pager-count="5" @current-change="currentChange"
...
.recent-post-item {
    ...
    overflow: hidden;
    border-radius: 10px;
    -webkit-animation: hideToShow 1s ease-in-out;
    animation: hideToShow 1s ease-in-out;
}
